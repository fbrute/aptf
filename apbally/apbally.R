...
#where datetime between '2012/01/01' and '2012/10/31'
...
pm10_1016query <- dbSendQuery(con, 
                              ...
                              paste("select year(datetime) as year, 
                                    ...
                                    from pm10",
                                    "where pmptp > 0 and hour(datetime) between 10 and 16
                                    order by date(datetime);
                                    "))
...
data_all_years <- merge(aotdata, pm10_24data, by="date",all.x = T, all.y = T)
data_all_years <- merge(data_all_years, pm10_1016data, by="date",all.x = T, all.y = T)
data_all_years <- merge(data_all_years, pm25data, by="date", all.x = T, all.y = T)
data_all_years <- data_all_years[order(data_all_years$date),]


# plot(data_all_years$pm10_24, type='b', col='blue',ylim=c(1,100), 
#      xlab=paste(c("days of",config$lblmonth," ",config$year)))
# title("pm10global(blue), pm10_1016h(violet),aot500(green), pm25(red)")
# points(data_all_years$pm10_1016, type='b', col='violet')
# points(data_all_years$aot500*100, type='b', col='green')
# points(data_all_years$pm25, type='b', col='red')

library(lattice)
data_all_years <- mutate(data_all_years, month= as.numeric(substr(date,6,7)))
xyplot(pm10_24 ~ aot500*100 | month, data = data_all_years, layout =c(4,3))
xyplot(pm10_1016 ~ aot500*100 | month, data = data_all_years, layout =c(4,3))
library(ggplot2)
qplot(pm10_24, aot500*100 , data = data_all_years, color=month, geom = c("point"))
# histogram
qplot(pm10_24, data = data_all_years, fill=month)
qplot(log(pm10_24), data= data_all_years, geom = "density")


#library(ggplot2)
#qplot(pm10_24, data=data)
data2 <- na.omit(select(data_all_years,pm10_24, aot500))
datacor24 <- cor(data2$pm10_24, data2$aot500)
# datacor242 <- lm(data2$pm10_24 ~ data2$aot500)
#View(datacor24)
data3 <- na.omit(select(data_all_years,pm10_1016, aot500))

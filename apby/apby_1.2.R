...
output = function(data, periode) {
        ...
        output2(periode, data, "aot500", "pm10_24")
...
output2 = function(periode, data2k12, x_string, y_string) {
        ...
        data2k12 = na.omit(subset(data2k12, select =c(eval(x_string), eval(y_string))))
        ...
        fname = paste(basename, "png",sep='')
        ggsave(g, file=fname)
}

output1016 = function(periode, data2k12) {
        fname <- paste(as.character(date_start),
                       "_",
                       as.character(date_stop),
                       "_",

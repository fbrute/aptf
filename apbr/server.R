...
shinyServer(function (input, output) {
        ...
        output$apbrplot <- renderPlot({
                ...
                aotdata <- filter(aotyeardata, 
                                  (date  >= input$daterange[1] & date  <= input$daterange[2] ))
                #View(aotdata)
                # merge is left inner join from sql
                df.apbr <- merge(aotdata, pm10_24data, by=c("date","hour"),all.x = T, all.y = T)
                
                df.apbr <- merge(df.apbr, pm10_1016data, by=c("date","hour"),all.x = T, all.y = T)
                
                df.apbr <- merge(df.apbr, pm25data, by=c("date","hour"), all.x = T, all.y = T)
                
                
                df.apbr <- df.apbr[order(df.apbr$date, df.apbr$hour),]
                
                #View(df.apbr)
                #save(df.apbr, file="df.apbr_range.RData")
                #save(df.apbr, file="df.apbr.2k12.RData")
                ...
                g <- ggplot(df.apbr) 
                if (dbg) browser()
                #p <- g + facet_wrap(~ date, ncol = 5, scales = "free") + scale_x_continuous(breaks=seq(1,24))
                p <- g + facet_wrap(~ date, ncol = 5, scales = "free") + scale_x_continuous(breaks=seq(1,24)) + coord_cartesian(ylim=c(0, input$ylim))
                p <- p + geom_point(aes(hour, pm10_24), color="blue")  
                p <- p + geom_line(aes(hour, pm10_24), color="blue")
                
                p <- p + geom_point(aes(hour, pm10_1016), color="violet")
                p <- p + geom_line(aes(hour, pm10_1016), color="violet")
                
                p <- p + geom_point(aes(hour, aot500*100), color="green")
                p <- p + geom_line(aes(hour, aot500*100), color="green")
                
                p <- p + geom_point(aes(hour, pm25), color="red")
                p <- p + geom_line(aes(hour, pm25), color="red")
                ...
                p <- p  +  theme(axis.text.x = element_text(size = 8, angle=0, vjust=1))
                print(p)
        ...
        getData <- function(datatype=""){
                ...
                if (datatype == "aot")
                        QueryString <- paste( 
                                "select year(date) as year,", 
                                        ...
                                        "hour(subtime(time,'03:00:00')) as hour,",
                ...
                if (datatype == "pm10_24")
                        QueryString <-paste(
                                ...
                                "from pm10",
                                "where date(datetime) between", "'", input$daterange[1], "'" ,
                                "and" , "'", input$daterange[2], "'" ,
                                "and hour(datetime) > 0",
                                "and pmptp > 0", 
                                "group by date, hour(datetime)",
                                "order by date, hour(datetime)")
